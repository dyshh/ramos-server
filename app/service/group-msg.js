...
module.exports = app => {
    return class GroupMsgService extends app.Service {
        ...
        async getHistoryList({ groupId, page = 1, size = 10 }) {
            ...
            const sql =
                'SELECT g.id,g.from_user_id,g.to_group_id,g.message,g.created_at,u.avatar,u.name,u.status FROM group_msg as g inner join user as u on g.from_user_id = u.id where (g.to_group_id = ?) order by g.created_at desc limit ?,?';
